...
2. Bump `cache_version` = `DateTime.now().millisecondsSinceEpoch` vào `app_meta`.
3. Ghi JSON `{"cache_version": <long>, "triggers": {trigger → content}}` vào `<app_support>/snippets_cache.json` theo kiểu ghi file `.tmp` rồi rename (giảm nguy cơ IME đọc phải file dở dang sau này).
...
- Việc bump `cache_version` bằng timestamp: nếu đồng hồ hệ thống bị chỉnh lùi, version mới có thể nhỏ hơn version cũ. Phase 1 cần biết hành vi mong muốn khi version giảm (reload hay bỏ qua)?
